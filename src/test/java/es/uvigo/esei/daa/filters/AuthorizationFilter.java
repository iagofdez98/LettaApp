...
/**
 * This performs the Basic HTTP authentication following (almost) the same rules
 * as the defined in the web.xml file.
 * 
 * @author A Team
 */
@Provider
...
public class AuthorizationFilter implements ContainerRequestFilter {
	...
	public void filter(ContainerRequestContext requestContext) throws IOException {
		...
		if (auth == null) {
			...
			final byte[] decodedToken = Base64.getDecoder().decode(auth.substring(6));
			...
			if (userPass.length == 2) {
				try {
					if (this.dao.checkLogin(userPass[0], userPass[1])) {
						...
						if ((isEventsPath(requestContext))
								&& !user.getRole().equals("normal")) {
							requestContext.abortWith(createResponse());
						} else {
	...
	private static boolean isEventsPath(ContainerRequestContext context) {
		final List<PathSegment> pathSegments = context.getUriInfo().getPathSegments();
		return !pathSegments.isEmpty() && pathSegments.get(0).getPath().equals("events");
	}

	private static Response createResponse() {
		return Response.status(Status.UNAUTHORIZED).header(HttpHeaders.WWW_AUTHENTICATE, "Basic realm=\"LettaApp\"")
